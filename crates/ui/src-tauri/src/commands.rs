...
pub async fn list_instances(
    ...
    let mut queried_provider_names: Vec<String> = Vec::new();
    for (provider_name, result) in [r_aws, r_oracle, r_gcp, r_azure] {
        if let Some(instances) = result {
            queried_provider_names.push(provider_name.to_string());
    ...
    if let Some(ledger) = LedgerStore::open(&app_handle) {
        ...
        let queried_provider_refs: Vec<&str> =
            queried_provider_names.iter().map(|s| s.as_str()).collect();
        ledger.reconcile_terminated(&running_ids, &queried_provider_refs);
    }
