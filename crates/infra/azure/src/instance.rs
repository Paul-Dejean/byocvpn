...
pub async fn spawn_instance(
    ...
    debug!("[Azure] Provider registration confirmed for '{}'.", location);
    ...
    debug!(
        ...
        vm_name,
        if public_ip_v4.is_empty() { None } else { Some(&public_ip_v4) },
        if public_ip_v6.is_empty() { None } else { Some(&public_ip_v6) }
    );
...
pub async fn list_all_instances(client: &AzureClient) -> Result<Vec<InstanceInfo>> {
    ...
    debug!("[Azure] Found {} byocvpn instance(s) in subscription.", instances.len());
    Ok(instances)
}
