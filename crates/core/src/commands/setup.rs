    ...
    F: Fn(&str, SpawnStepStatus, Option<String>),
{
    ...
    for step in steps {
        ...
        if let Err(error) = provider.run_provision_account_step(&step.id).await {
            error!("Provision account step failed: step={} error={}", step.id, error);
...
where
    F: Fn(&str, SpawnStepStatus, Option<String>),
{
    info!("Running {} enable-region steps for region: {}", steps.len(), region);
    for step in steps {
        debug!("Running enable-region step: {} for region: {}", step.id, region);
        on_step_progress(&step.id, SpawnStepStatus::Running, None);
        if let Err(error) = provider.run_enable_region_step(&step.id, region).await {
            error!("Enable-region step failed: step={} region={} error={}", step.id, region, error);
            on_step_progress(&step.id, SpawnStepStatus::Failed, Some(error.to_string()));
            return Err(error);
        }
        debug!("Enable-region step completed: {} for region: {}", step.id, region);
        on_step_progress(&step.id, SpawnStepStatus::Completed, None);
    }
